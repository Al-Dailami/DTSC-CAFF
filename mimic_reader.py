...
class MIMICReader(object):
    ...
    def __init__(self, data_path, max_len=335, device=None):
        ...
        self.no_ts_features = (pd.read_csv(self._timeseries_path, index_col='patient', nrows=1).shape[1] - 2)//2
        self.no_flat_features = self.flat.shape[1]
        self.no_daig_features = 1  # no diagnoses for MIMIC

        self.patients = list(self.labels.index)
        self.no_patients = len(self.patients)

        self.ratio = Counter(self.labels_mort)
    ...
    def pad_sequences(self, ts_batch):
        ...
        padded = [patient[:max_len] + [[0] * (self.no_ts_features * 2 + 2)] * (max_len - len(patient[:max_len])) for patient in ts_batch]
        # seq_lengths = [len(x) for x in ts_batch]
        # max_len = max(seq_lengths)
        # padded = [patient + [[0] * (self.no_ts_features * 2 + 2)] * (max_len - len(patient)) for patient in ts_batch]
...
if __name__=='__main__':
    MIMIC_reader = MIMICReader('./MIMIC_data/train')
